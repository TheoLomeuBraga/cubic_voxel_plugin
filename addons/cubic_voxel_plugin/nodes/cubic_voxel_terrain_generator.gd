...
var cube_map : Dictionary[Vector3i,int]
func cube_map_has(pos : Vector3i) -> bool:
	return cube_map.has(pos)

func cube_map_get(pos : Vector3i) -> int:
	var ret : int = -1
	if cube_map.has(pos):
		ret = cube_map[pos]
	return ret

func cube_map_set(pos : Vector3i,value : int) -> void:
...
func should_generate_wall_between(pos_a : Vector3i,pos_b : Vector3i) -> bool:
	...
	if (cube_map_has(pos_a) and cube_map_has(pos_b)) and (is_transparent(cube_map_get(pos_a)) or is_transparent(cube_map_get(pos_b))) and (cube_map_get(pos_a) != cube_map_get(pos_b)) :
		return true
...
func generate_mesh_to(st : SurfaceTool,block : Vector3i) -> void:
	...
	if should_generate_wall_between(block,block+Vector3i.UP):
		for v : Vector3 in plane_up:
			...
			st.set_uv(Vector2(vp.x,vp.z) - Vector2(.5,.5))
			st.add_vertex(vp)
	...
	if should_generate_wall_between(block,block+Vector3i.FORWARD):
		for v : Vector3 in plane_foward:
			...
			var vp : Vector3 = Vector3(v.x + block.x, v.y + block.y, v.z + block.z)
			st.set_uv(Vector2(vp.x,-vp.y) - Vector2(.5,.5))
			st.add_vertex(vp)
	...
	if should_generate_wall_between(block,block+Vector3i.LEFT):
		for v : Vector3 in plane_left:
			...
			var vp : Vector3 = Vector3(v.x + block.x, v.y + block.y, v.z + block.z)
			st.set_uv(Vector2(vp.z,-vp.y) - Vector2(.5,.5))
			st.add_vertex(vp)
